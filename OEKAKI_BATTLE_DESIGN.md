...
4. ツールバー（コンパクト、2段まで）:
   - **13色パレット**: 黒・灰・茶・赤・オレンジ・黄・黄緑・緑・水色・青・紫・ピンク・白（タップで選択、選択中は枠強調）。白は消しゴムより細く塗りつぶしを抜けるので、ハイライトや修正に使える
   - **太さスライダー**: `<input type="range" min="2" max="24" step="2">`。ラベル横に**実寸プレビュー**（いまの色・太さの丸）を表示
   - **ペンサイズ枠**: 描画中はポインタ位置に線と同じ直径のリング（`#okPenCursor`）を重ねて表示（白や薄い色でも太さが分かる）
